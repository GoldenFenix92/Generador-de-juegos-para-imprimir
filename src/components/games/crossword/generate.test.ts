...
describe("generateCrossword", () => {
  it("generates a grid with correct dimensions", () => {
    ...
    expect(result.grid[0].length).toBe(9);
  });
  ...
  it("has matching grid and blocks dimensions", () => {
    ...
    expect(result.blocks.length).toBe(9);
    expect(result.blocks[0].length).toBe(9);
  });
  ...
  it("maps difficulty to grid size", () => {
    const easy = generateCrossword({ size: 7, difficulty: "easy", wordCount: 5, generationMode: "random" });
    const medium = generateCrossword({ size: 9, difficulty: "medium", wordCount: 7, generationMode: "random" });
    const hard = generateCrossword({ size: 11, difficulty: "hard", wordCount: 9, generationMode: "random" });
    const expert = generateCrossword({ size: 13, difficulty: "expert", wordCount: 11, generationMode: "random" });

    expect(easy.width).toBe(7);
    expect(medium.width).toBe(9);
    expect(hard.width).toBe(11);
    expect(expert.width).toBe(13);
  });
